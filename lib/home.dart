...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void _handleSubmitted(String value) async {
    if (_videocontroller.value.isInitialized && !_isPlaying) {
                    ...
                    return Container(
                      ...
                      child: VideoPlayerWidget(
                        ...
                        onControllerInitialized: (controller) {
                          // เก็บ controller ในตัวแปรเพื่อใช้ใน _handleSubmitted
                          _videocontroller =
                              controller; // หรือใช้ตัวแปรใน StatefulWidget ของคุณ
                        },
                      ),
                    );
                  } else {
...
class VideoPlayerWidget extends StatefulWidget {
  ...
  final ValueChanged<VideoPlayerController>
      onControllerInitialized; // Callback ใหม่
...
class _VideoPlayerWidgetState extends State<VideoPlayerWidget> {
  ...
  void initState() {
    ...
    controller = VideoPlayerController.file(widget.file)
      ..initialize().then((_) {
        setState(() {
          // อัปเดต UI เมื่อวิดีโอพร้อมเล่น
        });
        widget.onControllerInitialized(controller); // ส่ง controller กลับไป
        controller.play(); // เริ่มเล่นวิดีโอ
        controller.setLooping(true); // ตั้งค่าให้ไม่เล่นซ้ำ
